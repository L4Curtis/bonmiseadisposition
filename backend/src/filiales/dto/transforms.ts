 ...
 *
 * Copie de `equipment/dto/transforms.ts` (même logique, dupliquée pour
 * garder les deux modules indépendants — cf. périmètre exclusif du lot).
 */
...
/**
 * Variante pour un champ réellement facultatif (adresse, SIRET, nom
 * affiché…) : trim, puis une chaîne devenue vide est ramenée à `undefined`
 * plutôt que rejetée par `@IsNotEmpty()` — une valeur `undefined` signifie
 * « champ non fourni, ne pas modifier » côté import (voir filiales-import.ts).
 */

...
describe('AttachmentsService', () => {
  ...
  describe('create', () => {
    ...
    it('rejects upload on an anonymized bon', async () => {
      (prisma.bon.findUnique as Mock).mockResolvedValue({ id: 'b1', reference: 'BMD-1', anonymizedAt: new Date() });
  ...
  describe('download', () => {
    it('decrypts and returns the buffer for a matching bon', async () => {
      ...
      (prisma.attachment.findUnique as Mock).mockResolvedValue({
        id: 'att-1', bonId: 'b1', storedPath: 'b1_restitution_1.png.enc', filename: 'photo.png', mimeType: 'image/png',
      });
      const fsp = fsPromisesModule as unknown as { readFile: Mock };
    ...
    it('rejects when the attachment belongs to another bon', async () => {
      (prisma.attachment.findUnique as Mock).mockResolvedValue({ id: 'att-1', bonId: 'OTHER', storedPath: 'x.enc' });
  ...
  describe('remove', () => {
    it('deletes the row and audits', async () => {
      (prisma.attachment.findUnique as Mock).mockResolvedValue({ id: 'att-1', bonId: 'b1', storedPath: 'x.png.enc', filename: 'photo.png' });
      (prisma.attachment.delete as Mock).mockResolvedValue({});
  ...
  describe('purgeForBon', () => {
    it('deletes all attachment rows for a bon', async () => {
      (prisma.attachment.findMany as Mock).mockResolvedValue([{ id: 'a', storedPath: 'a.enc' }]);
      (prisma.attachment.deleteMany as Mock).mockResolvedValue({ count: 1 });
